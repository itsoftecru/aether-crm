...
import type { BendType, DrawingAttachment, DrawingElement, DrawingPoint, DrawingProduct, DrawingTool, ProductProfile, ProfileSegment } from '@/types/crm';
...
const DEFAULT_SEGMENTS: ProfileSegment[] = [
  { id: 'segment-1', lengthMm: 15, angleDeg: 0, bendType: 'hem', hemSizeMm: 15, hemDirection: 'inside', label: 'Завальцовка' },
  { id: 'segment-2', lengthMm: 20, angleDeg: 90, bendType: 'bend', label: 'Полка' },
  { id: 'segment-3', lengthMm: 100, angleDeg: 0, bendType: 'straight', label: 'Лицевая часть' },
  { id: 'segment-4', lengthMm: 20, angleDeg: 90, bendType: 'bend', label: 'Полка' },
];
...
function createDrawingProducts(elements: DrawingElement[]): DrawingProduct[] {
  return elements
...
function renderElementToSvg(element: DrawingElement): string {
  ...
  if (element.tool === 'line') return `<line x1="${element.start.x}" y1="${element.start.y}" x2="${element.end.x}" y2="${element.end.y}" stroke="${stroke}" stroke-width="2" fill="none"/>`;
...
function createPayload(elements: DrawingElement[], title: string): DrawingSavePayload {
  const products = createDrawingProducts(elements);
...
export function DrawingEditor({ dealId, dealTitle, initialDrawing, onSave, onExportPdf, onClose }: DrawingEditorProps) {
  const [activeTool, setActiveTool] = useState<DrawingTool>('profile');
  ...
  const [profile, setProfile] = useState<ProductProfile>({ name: 'Отлив', segments: DEFAULT_SEGMENTS, lengthMm: 2000, quantity: 3, material: 'Оцинкованная сталь', thicknessMm: 0.5, color: 'RAL 9003', notes: '' });
  const title = useMemo(() => initialDrawing?.title ?? `Чертеж ${dealTitle}`, [dealTitle, initialDrawing?.title]);
  const selectedElement = elements.find((element) => element.id === selectedElementId) ?? null;
  const products = useMemo(() => createDrawingProducts(elements), [elements]);
  ...
  const updatePreview = (start: DrawingPoint, end: DrawingPoint) => {
    ...
    setPreviewElement({ id: `preview-${activeTool}`, tool: activeTool, start, end, text: activeTool === 'dimension' ? `${length} мм` : undefined });
  };

  const addProfileAtPoint = (point: DrawingPoint) => {
    const element: DrawingElement = { id: `profile-${dealId}-${Date.now()}`, tool: 'profile', start: point, end: point, profile: { ...profile, segments: profile.segments.map((segment) => ({ ...segment, id: `${segment.id}-${Date.now()}` })) } };
    setElements((current) => [...current, element]);
    setSelectedElementId(element.id);
  };

  const updateProfileSegment = (segmentId: string, patch: Partial<ProfileSegment>) => setProfile((current) => ({ ...current, segments: current.segments.map((segment) => segment.id === segmentId ? { ...segment, ...patch } : segment) }));
  const payload = createPayload(elements, title);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 p-4 backdrop-blur-sm">
      <div className="mx-auto max-w-7xl rounded-3xl bg-slate-50 p-4 shadow-2xl">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">Редактор чертежей</p><h2 className="mt-1 text-2xl font-bold text-slate-950">{title}</h2><p className="mt-1 text-sm text-slate-600">Оранжевая точка — реальный курсор, синяя — точка привязки. Профили задаются в миллиметрах.</p></div>
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[320px_1fr]">
          <aside className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
            <section><h3 className="font-bold text-slate-950">Параметрический профиль</h3><div className="mt-3 grid gap-2 text-sm"><input value={profile.name} onChange={(event) => setProfile((current) => ({ ...current, name: event.target.value }))} className="rounded-xl border px-3 py-2" placeholder="Название" /><div className="grid grid-cols-2 gap-2"><input type="number" value={profile.lengthMm} onChange={(event) => setProfile((current) => ({ ...current, lengthMm: Number(event.target.value) || 0 }))} className="rounded-xl border px-3 py-2" placeholder="Длина" /><input type="number" value={profile.quantity} onChange={(event) => setProfile((current) => ({ ...current, quantity: Number(event.target.value) || 1 }))} className="rounded-xl border px-3 py-2" placeholder="Кол-во" /></div><div className="grid grid-cols-2 gap-2"><input value={profile.material} onChange={(event) => setProfile((current) => ({ ...current, material: event.target.value }))} className="rounded-xl border px-3 py-2" placeholder="Материал" /><input type="number" step="0.1" value={profile.thicknessMm} onChange={(event) => setProfile((current) => ({ ...current, thicknessMm: Number(event.target.value) || 0 }))} className="rounded-xl border px-3 py-2" placeholder="Толщина" /></div><input value={profile.color} onChange={(event) => setProfile((current) => ({ ...current, color: event.target.value }))} className="rounded-xl border px-3 py-2" placeholder="Цвет" /></div></section>
            <section><h4 className="text-sm font-bold text-slate-950">Сегменты, углы и завальцовки</h4><div className="mt-2 space-y-2">{profile.segments.map((segment, index) => <div key={segment.id} className="rounded-xl bg-slate-50 p-2 text-xs"><div className="mb-2 font-bold text-slate-700">#{index + 1} {segment.label}</div><div className="grid grid-cols-2 gap-2"><input type="number" value={segment.lengthMm} onChange={(event) => updateProfileSegment(segment.id, { lengthMm: Number(event.target.value) || 0 })} className="rounded-lg border px-2 py-1" /><input type="number" value={segment.angleDeg} onChange={(event) => updateProfileSegment(segment.id, { angleDeg: Number(event.target.value) || 0 })} className="rounded-lg border px-2 py-1" /></div><select value={segment.bendType} onChange={(event) => updateProfileSegment(segment.id, { bendType: event.target.value as BendType })} className="mt-2 w-full rounded-lg border px-2 py-1"><option value="straight">Прямая</option><option value="bend">Гиб</option><option value="hem">Завальцовка</option><option value="lock">Замок</option><option value="dripEdge">Капельник</option></select>{segment.bendType === 'hem' ? <div className="mt-2 grid grid-cols-2 gap-2"><input type="number" value={segment.hemSizeMm ?? segment.lengthMm} onChange={(event) => updateProfileSegment(segment.id, { hemSizeMm: Number(event.target.value) || 0 })} className="rounded-lg border px-2 py-1" /><select value={segment.hemDirection ?? 'inside'} onChange={(event) => updateProfileSegment(segment.id, { hemDirection: event.target.value as 'inside' | 'outside' })} className="rounded-lg border px-2 py-1"><option value="inside">Внутрь</option><option value="outside">Наружу</option></select></div> : null}</div>)}</div></section>
            <section><h4 className="text-sm font-bold text-slate-950">Спецификация</h4>{products.length > 0 ? <ul className="mt-2 space-y-2 text-xs text-slate-600">{products.map((product) => <li key={product.id} className="rounded-xl bg-slate-50 p-2"><b>{product.name}</b> {product.profileFormula} мм · L={product.lengthMm} мм · {product.quantity} шт</li>)}</ul> : <p className="mt-2 text-xs text-slate-500">Поставьте инструмент «Профиль» и кликните по полю.</p>}</section>
